...
void CGibbsMLDA::Initilize( int numTopic , int numDocument , int numModal , int *modalDim )
{
	...
	*/
	...
	for (size_t m=0; m<numModal; m++)
	{
		m_aliasTables_mw[m] = new CAliasTable[ modalDim[m] ];
	}
}
...
void CGibbsMLDA::ReleaseBuffer()
{
	...
	for (size_t m = 0; m < m_numModal; m++)
	{
		delete[] m_aliasTables_mw[m];
	}
}
...
int CGibbsMLDA::SamplingMH(int d, int m, int l, bool fixed)
{
	...
	if (t!=s)
	{
		int nsw = m_N_mwz[m][w][s];
		int ntw = m_N_mwz[m][w][t];
		int ns = m_N_z[s];
		int nt = m_N_z[t];
...
{
	double *Pwz = new double[m_numTopic];
	...
	for (int m = 0; m<m_numModal; m++)
	{
		...
		for (int w = 0; w<W; w++)
		{
			for (int z = 0; z<m_numTopic; z++)
			{
				Pwz[z] = (m_N_mwz[m][w][z] + beta) / (m_N_mz[m][z] + W * beta);
			}

			m_aliasTables_mw[m][w].BuildTable(Pwz, m_numTopic);
		}
	}
...
double CGibbsMLDA::UpdateMH(bool fixed)
{
	double plik = 0;
	clock_t start = clock();
